...
class HeatSafeAppTests(unittest.TestCase):
    ...
    def test_production_mode_renders_primary_controls_and_refreshes_locally(self):
        environment = {
        ...
        with mock.patch.dict(os.environ, environment, clear=False):
            ...
            self.assertEqual(
                ...
                ["Experience mode", "Decision zone"],
            )
            self.assertTrue(
                any(
                    "PRODUCTION" in item.value
            ...
            self.assertNotIn("Replay run", [item.label for item in app.selectbox])
            self.assertEqual(
    ...
    def test_accelerated_production_mode_loads_verified_window(self):
        ...
        with mock.patch.dict(os.environ, environment, clear=False):
            ...
            app.run()
            app.selectbox[0].select("accelerated-production")
            app.run()
            self.assertFalse(app.exception)
            self.assertTrue(
                any(
                    "ACCELERATED PRODUCTION" in item.value
                    for item in app.markdown
                )
            )
